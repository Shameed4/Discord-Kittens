...
type Coordinator interface {
	// tries to take ownership of a lobby, returning the address if it already exists in another lobby
	Acquire(name string) (owned bool, ownerAddr string, err error)
	// finds which node owns this lobby without trying to claim it
...
func (coord LocalCoordinator) Acquire(name string) (owned bool, ownerAddr string, err error) {
	return true, "", nil
}
